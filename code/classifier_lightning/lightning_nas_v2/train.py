...
def parse_args():
    ...
    parser.add_argument(
        "--conv51_channels", type=int, required=True, help="channels in conv 5, 1 stage"
    )
    parser.add_argument(
...
def run_training_job():
    ...
    if config.MODEL == "BED":
        ...
        model = BED_CLASSIFIER(
            ...
            conv51_channels=args.conv51_channels,
    ...
    with io.capture_output() as captured:
        val_smoke_f1 = trainer.validate(model, val_loader)[0]['val_smoke_f1']
    logger.log_metrics({'val_smoke_f1': val_smoke_f1})
    ...
    # Print outputs
    print(f'train time: {train_time}, val smoke f1: {val_smoke_f1}, num params: {num_params}')

...
import numpy as np
import cv2
import modules.yolov1_config as config
...
# ______________________________________________________________ #
# _____________ Preprocess Image: plot and prediction  _________ #
# ______________________________________________________________ #
def preprocess_image_for_plotting(img_file):
    img = cv2.imread(img_file)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  
    img = cv2.resize(img, (config.IMG_W, config.IMG_H), interpolation = cv2.INTER_NEAREST)
    return img

def preprocess_image_for_prediction(img_file):
    img = cv2.imread(img_file)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  
    img = cv2.resize(img, (config.IMG_W, config.IMG_H), interpolation = cv2.INTER_NEAREST)
    img = (img / 256.)
    img = np.expand_dims(img, axis=0) # Batch Dim
    img = np.transpose(img, (0, 3, 1, 2)) # Pytorch (N, C, H, W)
